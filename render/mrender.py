...
Single frame:   mrender    [options] file.ma|mb [imagefile]
Frame range:    mrender -e [options] file.ma|mb

options:        -w pixels       Output width
                -h pixels       Output height
                -F frame        Single frame
                -r render       Render name
                -l layer		Render a specified layer
                -o output       Output name specification
                -proj project   Project location
                -v              Run in verbose mode
                -m              Abort render when missing texture
...
def validate_args(args):
    ...
    mafiles = []
    for f in args.file:
        if '.' in f and f.split('.')[-1] in ('ma', 'mb'):
            mafiles.append(f)

    if not mafiles:
        return 'Missing .ma motion file name.'
    if len(mafiles) > 1:
        return 'Too many .ma motion file names: %s.' % (' '.join(mafiles))
    if not os.path.isfile(mafiles[0]):
        return 'Cannot find file %s.' % mafiles[0]
